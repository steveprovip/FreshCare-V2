        ...
        public IActionResult DoanhThu(DateTime? tuNgay, DateTime? denNgay)
        {
            ...
            try
            {
                using (var conn = DatabaseHelper.GetConnection(_connectionString))
                {
                    ...
                    // Bao gồm sản phẩm có tồn kho nhưng chưa bán hoặc bán ít nhất
                    ...
                    string sqlTopBanCham = $@"SELECT TOP 5 sp.MaSP, sp.TenSP, sp.DonViTinh,
                                                   ISNULL(sold.TongSoLuong, 0) AS TongSoLuong,
                                                   ISNULL(sold.TongDoanhThu, 0) AS TongDoanhThu
                                            FROM SanPham sp
                                            INNER JOIN LoHang lh ON sp.MaSP = lh.MaSP AND lh.SoLuongTon > 0 AND lh.TrangThai != N'Đã Hủy'
                                            OUTER APPLY (
                                                SELECT SUM(ct2.SoLuong) AS TongSoLuong,
                                                       SUM(ct2.SoLuong * ct2.DonGia) AS TongDoanhThu
                                                FROM ChiTietXuat ct2
                                                INNER JOIN LoHang lh2 ON ct2.MaLo = lh2.MaLo
                                                INNER JOIN PhieuXuat px2 ON ct2.MaPhieuXuat = px2.MaPhieuXuat
                                                WHERE lh2.MaSP = sp.MaSP 
                                                      AND px2.LoaiPhieu = N'Bán Hàng'
                                                      AND px2.NgayXuat >= @TuNgay AND px2.NgayXuat <= @DenNgay
                                            ) sold
                                            WHERE sp.TrangThai = N'HoatDong'
                                                  {excludeClause}
                                            GROUP BY sp.MaSP, sp.TenSP, sp.DonViTinh, sold.TongSoLuong, sold.TongDoanhThu
                                            ORDER BY ISNULL(sold.TongSoLuong, 0) ASC, sp.TenSP ASC";
